...
func RenderBreadcrumb(match *matchmodel.MatchWithPlayers, set *setmodel.Set, game *gamemodel.Game, point *pointmodel.Point, play *playmodel.Play) (template.HTML, error) {
	return breadcrumbComponent.Render(map[string]any{"Match": match, "Set": set, "Game": game, "Point": point, "Play": play})
}